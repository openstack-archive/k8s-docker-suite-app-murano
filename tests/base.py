...
class MuranoTestsBase(testtools.TestCase, clients.ClientsBase):
    ...
    def get_k8s_instances(self, environment):
        k8s_instances = {'gateways': [], 'minions': []}
        for gw in environment.services[0]['gatewayNodes']:
            if gw['instance']['floatingIpAddress']:
                k8s_instances['gateways'].append(
                    gw['instance']['floatingIpAddress']
                )
        for minion in environment.services[0]['minionNodes']:
            if minion['instance']['floatingIpAddress']:
                k8s_instances['minions'].append(
                    minion['instance']['floatingIpAddress']
                )
