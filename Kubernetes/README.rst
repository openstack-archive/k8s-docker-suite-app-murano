...
* `Publishing services`_:  ClusterIP Type
...
runtime has been added. There are more runtimes planned to be added in future.

Kubernetes Cluster deployed by Murano curently support only Docker runtime.
...
Kubernetes Cluster deployed by Murano support Calico networking by default.
 ...
 * `Download and install <http://kubernetes.io/docs/getting-started-guides/minikube/#install-kubectl>`_
the ``kubectl`` executable based on OS of the choice.
