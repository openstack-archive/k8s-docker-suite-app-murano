...
Kubernetes cluster deployed by Murano consists of following VMs that can be
...
Each gateway node runs confd and HAProxy service. When end user deploys an
...
Murano deploys etcd on every node. For example if you cluster consist of
single instances of Master, Node and Gateway, etcd will have 3 node cluster.
