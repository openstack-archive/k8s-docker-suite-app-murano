...
* **etcd** - distributed key-value store, which ensures Calico can always build
  an accurate network, used primerly for data storage, communication
...
* **Confd**, a templating process to auto-generate configuration for BIRD,
  monitors the etcd store for any changes to BGP configuration such as IPAM
  information. Confd also dynamically generates BIRD configuration files based
  on data from etcd and triggers automatically as updates are applied to data.
