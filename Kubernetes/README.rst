...
containers:Murano project consists of several source code repositories:
...
individual Kubernetes resources and workloads, such as Deployments, Daemonsets, Replica sets, Jobs, Replication controllers, corresponding Services and Pods.
...
Deploy and manage applications using Kubernetes command-line tool - ``kubectl`` from you laptop or any local environment:
...
Additionally, it is possible to access ``kubectl cli`` from Master Node (kube-1), where ```kubectl cli``` installed and configured by default.
